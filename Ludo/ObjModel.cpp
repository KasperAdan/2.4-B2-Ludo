...
void ObjModel::init()
{
	// create temp list for the vertices
	std::vector<tigl::Vertex> verts;

	for (auto& g : groups) {
		// set material texture, if available
		if (materials.at(g->materialIndex)->texture) {
			materials.at(g->materialIndex)->texture->bind();
		}

		// set material color, if available
		tigl::shader->setColorMult(materials.at(g->materialIndex)->color);
		...
		for (auto f : g->faces) {
			...
			for (auto v : f.vertices) {
				// emit vertex
				verts.push_back(tigl::Vertex::PTN(vertices.at(v.position), texcoords.at(v.texcoord), normals.at(v.normal)));
			}
		}
	}
...
void ObjModel::setColor(glm::vec4 color)
{
...
void ObjModel::draw()
{
	if (materials.at(0)->texture)
		materials.at(0)->texture->bind();
	else
		glBindTexture(GL_TEXTURE_2D, 0);

	tigl::shader->setColorMult(materials.at(0)->color);
